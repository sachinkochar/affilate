...
componentDidMount(){
    this.props._search(this.state.pageNo);
    this.props._tags();
}
...
handleClick(tag) {
    ...
    if(tags.indexOf(tag) < 0){
        tags.push(tag);
        this.props._filter(tags, this.state.pageNo)
    }
}
                                    ...
                                    <nav className="float-right">
                                        <ul className="pagination pagination-circle pg-blue mb-0">
                                            ...
                                            {/* <li className="page-item active">
                                                <span
                                                    className="pagination-items page-link"
                                                    mdbWavesEffect
                                                    onClick={() => {this.setState({pageNo: i})}}
                                                >
                                                    {i}
                                                </span>
                                            </li> */}
...
function mapStateToProps(state) {
  ...
  function mapDispatchToProps(dispatch, props) {
    return {
      _search: (page) => {
        dispatch(_searchAction(page));
      },
      _tags: () => {
        dispatch(_tagsAction())
      },
      _filter: (tags, page) => {
        dispatch(_searchFilterAction(tags, page))
      }
    }
  }
